document.addEventListener("DOMContentLoaded", function () {
        ...
        try {
            if (!isNaN(arrivalValue) && !isNaN(departureValue) && departureValue >= arrivalValue) {
                ...
                options.forEach(option => {
                    option.style.display = ''; // Aseg├║rate de mostrar la opci├│n
                    option.style.backgroundColor = ''; // Limpia colores anteriores
                    option.disabled = false; // Habilita la opci├│n
                });
                ...
                const arrivalMonth = (arrivalValue.getMonth() + 1).toString().padStart(2, '0'); // Asegura que el mes tenga dos d├¡gitos
                const arrivalDay = arrivalValue.getDate().toString().padStart(2, '0'); // Asegura que el d├¡a tenga dos d├¡gitos
                const arrivalDate = `${arrivalYear}-${arrivalMonth}-${arrivalDay}`;

                const departureYear = departureValue.getFullYear();
                const departureMonth = (departureValue.getMonth() + 1).toString().padStart(2, '0'); // Asegura que el mes tenga dos d├¡gitos
                const departureDay = departureValue.getDate().toString().padStart(2, '0'); // Asegura que el d├¡a tenga dos d├¡gitos
                ...
                results.forEach(result => {
                    ...
                    if (result.available) {
                        option.style.backgroundColor = 'lightgreen'; // Marca las caba├▒as disponibles
                        option.disabled = false;
                    } else {
                        // option.style.backgroundColor = 'lightcoral'; // Marca las caba├▒as no disponibles
    ...
    tipologiaSelect.addEventListener('change', function () {
        ...
        numeroPersonasSelect.innerHTML = '<option value="" selected disabled>Selecciona el n├║mero de personas --</option>';
    ...
    async function obtenerTotalReserva() {
        ...
        if (arrivalDate.value && departureDate.value && tipologiaSelect.value && selectedPax.value !== "") {
            ...
            try {
                ...
                console.log("Total m├íximo permitido con comisiones: ", totalPrecios)
