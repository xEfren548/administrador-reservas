...
async function fetchRestrictedDates() {
    try {
        const response = await fetch('/api/calendario-bloqueofechasinversionistas');
        if (!response.ok) {
...
function createCalendar(month, year, calendarContainerId, chaletId) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        let description = "Fecha bloqueda para inversionistas"
...
$('#delete-prices-btn').on('click', async () => {
    ...
    for (const fecha of fechas){
        ...
        try {
            ...
            const response = await fetch(`/api/calendario-bloqueofechasinversionistas?fecha=${formattedDate}&habitacionId=${chaletId}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                }
